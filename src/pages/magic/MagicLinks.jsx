...
const MagicLinksDashboard = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const linksData = linksRes?.success && Array.isArray(linksRes.data) ? linksRes.data : [];
      ...
      // ✅ SAFETY CHECK: Extract unique projects safely
      if (linksData.length > 0) {
        const uniqueProjectIds = [...new Set(
          linksData
            .filter(link => link?.project?._id)
            .map(link => link.project._id)
        )];
      ...
      setMagicLinks([]); // ✅ Reset to empty array on error
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSelectAll = () => {
    if (selectedLinks.length === magicLinks.length) {
      ...
      setSelectedLinks(magicLinks.map(link => link?._id).filter(Boolean));
    }
  };
  ...
  const getStatusBadge = (link) => {
    if (!link) return <span className="badge bg-secondary">Unknown</span>;
  ...
  const filteredLinks = magicLinks.filter(link => {
    ...
    const matchesSearch = 
      (link.email || '').toLowerCase().includes(searchLower) ||
      (link.project?.name || '').toLowerCase().includes(searchLower);
            ...
            <div className="col-md-4">
              ...
              >
                ...
                {projects.map(projectId => {
                  const link = magicLinks.find(l => l?.project?._id === projectId);
                  return (
              ...
              <tbody>
                {filteredLinks.length === 0 ? (
                  ...
                  filteredLinks.map((link) => {
                    // ✅ SAFETY CHECK: Skip if link is invalid
                    if (!link || !link._id) {
                      ...
                      <tr key={link._id}>
                        ...
                        <td>
                          <div className="d-flex align-items-center">
                            ...
                            <div>
                              <div className="fw-bold small">{link.email || 'No email'}</div>
                        ...
                        <td>
                          {link.status === 'active' && !link.usedAt && link.expiresAt ? (
                            ...
                            <div key={approval._id} className="list-group-item">
                              <div className="form-check">
                                ...
                                <label className="form-check-label d-block" htmlFor={approval._id}>
                                  <span className="fw-bold">{approval.title || 'Untitled'}</span>
                                  <small className="text-muted d-block">
                                    Status: {approval.status || 'pending'} • Type: {approval.type || 'text'}
                                  </small>
